//http://www.porcupyne.org/docs/browse_source/JavaServlet/com/oreilly/servlet/multipart/MultipartParser.java.html
...
package framework.ressource.upload;
...
/**
 * A utility class to handle <code>multipart/form-data</code> requests, the kind of requests that support file uploads.  This class uses a "pull" model where the reading of incoming files and parameters is controlled by the client code, which allows incoming files to be stored into any <code>OutputStream</code>.  If you wish to use an API which resembles <code>HttpServletRequest</code>, use the "push" model <code>MultipartRequest</code> instead.  It's an easy-to-use wrapper around this class. <p> This class can receive arbitrarily large files (up to an artificial limit you can set), and fairly efficiently too. It cannot handle nested data (multipart content within multipart content) or internationalized content (such as non Latin-1 filenames). <p> It also optionally includes enhanced buffering and Content-Length limitation.  Buffering is only required if your servlet container is poorly implemented (many are, including Tomcat 3.2), but it is generally recommended because it will make a slow servlet container a lot faster, and will only make a fast servlet container a little slower.  Content-Length limiting is usually only required if you find that your servlet is hanging trying to read the input stram from the POST, and it is similarly recommended because it only has a minimal impact on performance. <p> See the included <a href="upload.war">upload.war</a> for an example of how to use this class. <p> The full file upload specification is contained in experimental RFC 1867, available at <a href="http://www.ietf.org/rfc/rfc1867.txt"> http://www.ietf.org/rfc/rfc1867.txt</a>.
 * @see com.oreilly.servlet.MultipartRequest
 * @author  Jason Hunter
 * @author  Geoff Soutter
 * @version  1.3, 2001/03/23, added IE5 bug workaround supporting \n as line ending,  thanks to Michael Alyn Miller
 * @version  1.3, 2001/01/22, added support for boundaries surrounded by quotes  and content-disposition after content-type,  thanks to Scott Stark
...
public class MultipartParser {
  ...
  public Part readNextPart() throws IOException {
    ...
    Enumeration lEnum = headers.elements();
    while (lEnum.hasMoreElements()) {
      String headerline = (String) lEnum.nextElement();
