...
public class FilePart extends Part {
  ...
  /**
 * Returns the name that the file was stored with on the remote system, or <code>null</code> if the user didn't enter a file to be uploaded. Note: this is not the same as the name of the form parameter used to transmit the file; that is available from the <code>getName</code> method.  Further note: if file rename logic is in effect, the file name can change during the writeTo() method when there's a collision with another file of the same name in the same directory.  If this matters to you, be sure to pay attention to when you call the method.
 ...
 */
  ...
  /**
 * Returns the full path and name of the file on the remote system, or <code>null</code> if the user didn't enter a file to be uploaded. If path information was not supplied by the remote system, this method will return the same as <code>getFileName()</code>.
